...
dados %>% 
    ...
    labs(title = "Proporção de respostas recebidas")
```
...
eleitos %>%  
    ...
    mutate(Respondeu = if_else(respondeu, "Sim", "Não")) %>% 
    ggplot(aes(x = reorder(uf, total), y = n, fill = Respondeu)) + 
    geom_col(position = "stack") + 
    coord_flip() + 
    labs(
        x = "",
...
```{r}
eleitos %>% 
    count(sg_partido, respondeu) %>%  group_by(sg_partido) %>% mutate(total = sum(n)) %>% ungroup() %>% 
    mutate(Respondeu = if_else(respondeu, "Sim", "Não")) %>% 
    ggplot(aes(x = reorder(sg_partido, total), y = n, fill = Respondeu)) + 
    geom_col(position = "stack") + 
    coord_flip() + 
    labs(
        x = "",
